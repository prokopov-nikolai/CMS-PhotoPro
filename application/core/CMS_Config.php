  ...
  public function save(){
      ...
      $fn = fopen(ROOT . '/' . $f,"w");
          ...
          if (isset($this->config[$matches[0][1]][$matches[0][2]])) {
            if ($this->config[$matches[0][1]][$matches[0][2]] === FALSE || $this->config[$matches[0][1]][$matches[0][2]] == 'FALSE') {
              $file[$key] = $matches[0][0] . " = FALSE;\n";
            } elseif ($this->config[$matches[0][1]][$matches[0][2]] === TRUE || $this->config[$matches[0][1]][$matches[0][2]] == 'TRUE') {
             ...
             $file[$key] = $matches[0][0] . " = '{$this->config[$matches[0][1]][$matches[0][2]]}';\n";
  				...
  				if (isset($this->config[$matches[0][1]])) {
  					if ($this->config[$matches[0][1]] === FALSE || $this->config[$matches[0][1]] == 'FALSE') {
              $file[$key] = $matches[0][0] . " = FALSE;\n";
  					} elseif ($this->config[$matches[0][1]] === TRUE || $this->config[$matches[0][1]] == 'TRUE') {
  						$file[$key] = $matches[0][0] . " = TRUE;\n";
